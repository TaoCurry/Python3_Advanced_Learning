...
1. 可变参数：将多个参数以tuple（元组）形式组合起来，然后使用。
>>> def calc1(number, *numbers):
	print(number)
	print(numbers)
...
2. 关键字参数：关键字参数允许你传入0个或任意个含参数名的参数，这些关键字参数在函数内部自动组装为一个dict。
>>> def clac2(**numbers):
	print(numbers)
>>> clac2(one=1, two=2, three=3)
{'one': 1, 'two': 2, 'three': 3}	# 组装为一个dict
